...
from versaprm.data import TokenizedPRMDataset
import evaluate
import numpy as np
...
def get_model(configs):
    ...
    if 'lora_config' in configs:
        ...
        if hasattr(model, 'peft_config') and model.peft_config:
            print('Model already has LoRA adapters - continuing fine-tuning')
        else:
            print('Model does not have LoRA adapters - adding new LoRA')
...
def get_datasets(configs, tokenizer):
    ...
    t_dataset = TokenizedPRMDataset(configs.train_data_path, 
                                    ...
                                    noise_data_ratio=configs.noise_data_ratio if 'noise_data_ratio' in configs else 0.,
                                    noise_process_ratio=configs.noise_process_ratio if 'noise_process_ratio' in configs else 0.,
                                    no_balance=configs.no_balance if "no_balance" in configs else False)
    e_dataset = TokenizedPRMDataset(configs.eval_data_path, 
                                    ...
                                    max_length=configs.max_length if 'max_length' in configs else None,
                                    task_type=configs.task_type,
                                    no_balance=configs.no_balance if "no_balance" in configs else False) if configs.eval_data_path is not None else None
