...
def main():
    ...
    for N_max in args.N_max_values:
        ...
        header = f"{'Method':<10} {'Overall':<{col_width}} {cat_headers}"
        
        # ==== BEST-OF-N TABLE ====
        print(f"\nBest-of-N Table for N_max = {N_max}")
        print(f"Settings: num_runs={args.num_runs}, strategy per model as specified, seed={args.seed}")
        print(header)
        print("-" * len(header))
        ...
        if first_model:
            ...
            mv_overall = f"{np.mean(mv_overall_runs):.{args.decimals}f}±{np.std(mv_overall_runs):.{args.decimals}f}"
            
            for cat in categories:
                if cat in all_model_results[first_model]:
                    scores = all_model_results[first_model][cat]['majority_vote']
                    result = f"{np.mean(scores):.{args.decimals}f}±{np.std(scores):.{args.decimals}f}"
                else:
                    result = "N/A"
                mv_results.append(f"{result:<{col_width}}")
            
            print(f"{'MV':<10} {mv_overall:<{col_width}} {' '.join(mv_results)}")
        ...
        for model_name in args.model_names:
            if model_name in all_model_results and all_model_results[model_name]:
                ...
                overall = f"{np.mean(overall_runs):.{args.decimals}f}±{np.std(overall_runs):.{args.decimals}f}"
                ...
                print(f"{model_name:<10} {overall:<{col_width}} {' '.join(cat_results)}")
            else:
                # Model has no results
                overall = "N/A"
                cat_results = [f"{'N/A':<{col_width}}" for _ in categories]
                print(f"{model_name:<10} {overall:<{col_width}} {' '.join(cat_results)}")
        ...
        if first_model:
            ...
            oracle_overall = f"{np.mean(oracle_overall_runs):.{args.decimals}f}±{np.std(oracle_overall_runs):.{args.decimals}f}"
            ...
            for cat in categories:
                if cat in all_model_results[first_model]:
                    ...
                    result = "N/A"
                oracle_results.append(f"{result:<{col_width}}")
            
            print(f"{'Pass@' + str(N_max):<10} {oracle_overall:<{col_width}} {' '.join(oracle_results)}")
        
        # ==== WEIGHTED MAJORITY VOTE TABLE ====
        print(f"\nWeighted Majority Vote Table for N_max = {N_max}")
        print(f"Settings: num_runs={args.num_runs}, strategy per model as specified, seed={args.seed}")
        print(header)
        print("-" * len(header))
