...
def create_equal_width_bins(datasets, num_bins, min_count_per_bin=10):
    """Create equal-width bins with minimum count per bin"""
    ...
    # Start with equal-width bins
    bin_width = (max_length - min_length) / num_bins
    
    # Create initial equal-width bin edges
    initial_edges = np.linspace(min_length, max_length + 0.01, num_bins + 1)
    
    # Count items in each bin to check minimum threshold
    sorted_lengths = sorted(lengths)
    valid_edges = [initial_edges[0]]  # Always include the first edge
    
    for i in range(1, len(initial_edges)):
        # Count how many items would be in this bin
        if i == len(initial_edges) - 1:
            # Last bin - include all remaining items
            count_in_bin = sum(1 for l in lengths if valid_edges[-1] <= l <= initial_edges[i])
        else:
            count_in_bin = sum(1 for l in lengths if valid_edges[-1] <= l < initial_edges[i])
        
        # If this bin has enough items, keep the edge
        if count_in_bin >= min_count_per_bin:
            valid_edges.append(initial_edges[i])
        # If not enough items, merge with previous bin by not adding the edge
        # (the next iteration will create a larger bin)
    
    # Ensure we always have at least 2 edges (1 bin)
    if len(valid_edges) < 2:
        valid_edges = [min_length, max_length + 0.01]
    
    # Make sure the last edge covers all data
    valid_edges[-1] = max_length + 0.01
    
    return np.array(valid_edges)

def evaluate_by_length_bins(datasets, bin_edges, strategy="mean", include_reward_free=True, min_count_per_bin=10):
    ...
    for i in range(len(bin_edges) - 1):
        ...
        # Only process bins with minimum count (or always include if no minimum specified)
        if len(bin_questions) >= min_count_per_bin or min_count_per_bin == 0:
            if bin_questions:
...
def main():
    ...
    parser.add_argument("--min_count_per_bin", type=int, default=100,
                       help="Minimum number of data points required per bin")
    ...
    print(f"Settings: N_max={args.N_max}, num_runs={args.num_runs}, seed={args.seed}")
    print(f"Binning: {args.num_bins} equal-width bins, min {args.min_count_per_bin} samples per bin")
    ...
    # Create equal-width bins with minimum count threshold
    bin_edges = create_equal_width_bins(all_model_datasets[best_model_idx], 
                                       args.num_bins, args.min_count_per_bin)
    print(f"\nCreated {len(bin_edges)-1} equal-width bins with boundaries: {bin_edges}")
    print(f"Minimum {args.min_count_per_bin} samples per bin required")
    ...
    for model_idx in range(len(args.input_dirs)):
        ...
        if all_model_datasets[model_idx]:
            bin_results = evaluate_by_length_bins(
                ...
                min_count_per_bin=0  # Don't filter here since we want consistent bins
            )
        else:
    ...
    print(f"\nPerformance by Average CoT Length per Question (Equal-width bins):")
